...
export const convertExchangeRate = (baseCurrency = USDCurrencyCode) => {
  ...
  return Object.keys(usdRates).reduce((rates: Record<string, number>, code) => {
    ...
    const newXRate: number = shouldSwapWithUSDRate
      ? usdXRate
      : usdRates[code] * xRateMultiplier
    const maxDigits = (usdRates[code] || usdXRate)
      .toString()
      .split('.')[1].length
...
export const getExchangeRate = (
  ...
  if (baseCurrency === USDCurrencyCode)
    return getUSDExchangeRates()[quoteCurrency]
...
export const calculateSum = (
  ...
  const maxDigits = Math.min(
    ...
    Math.max(...inputs.map(amount => getDecimalDigits(amount.value)))
  )
  const sumResult = inputs.reduce(
    (sum, amount) =>
      sum + amount.value * getExchangeRate(amount.currency, desiredCurrency),
    0
  )
