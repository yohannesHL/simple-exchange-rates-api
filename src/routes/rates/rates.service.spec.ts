import { allExchangeRates, mockedExchangeRateConversion } from '../../../test/fixtures/exchangeRates'
...
describe('Rates service ', () => {
  ...
  it('convertCurrencyAmount() returns expected result', async () => {
    const result = await convertCurrencyAmount({ amount: 14, from: 'GBP', to:'EUR'})
    expect(result)
      .toMatchObject({
        ok: true,
        data: {
